...
CHECKPOINT="./checkpoints/training/first-iteration/checkpoint-146485/model.safetensors"
...
class StreamingTokenDataset(IterableDataset):
	...
	def __iter__(self):
		...
		for sample in self.dataset:
			self.total += 1
			if self.is_noisy_data(sample["text"]):
				self.total_noisy_data += 1
				if self.total > 500 and self.total % 1000 == 0:
...
def main():
	...
	raw_train_dataset = load_from_disk("./dataset/untokenized/cc_pt_train").shuffle(seed=42)
	...
	raw_val_dataset = load_from_disk("./dataset/untokenized/cc_pt_val").shuffle(seed=42)
	...
	raw_test_dataset = load_from_disk("./dataset/untokenized/cc_pt_test").shuffle(seed=42)
	...
	training_args = TrainingArguments(
		output_dir="./checkpoints/training/second-iteration",
		max_steps = 1_000_000,
		num_train_epochs = 1,
		...
		max_grad_norm=1.0,
		learning_rate=best_params["lr"] * 0.2,
	...
	torch.save(trainer.model.state_dict(), "./model/dev/midnight_2.pt")
